...
const StateFile = (props) => {
    ...
    const getUTMAppendedLinks = async (utmObject) => {
        ...
        if (response.ok) {
            ...
            setUtmAppendedLinks(jsonData);
            console.log('UTM appended links', jsonData); // Log the JSON data received from the serve
            validateAll(jsonData, ut);
        } else {
    ...
    function validateEachUtmParameters(link, acc) {
        ...
        for (const param in acc) {
            if (searchParams[param] !== acc[param]) {
                ...
                errors.push({ emailName: searchParams.utm_term, expected: acc[param], actual: searchParams[param], param: param, resValue: false });
            } else {
                //  console.log(searchParams[param]);
                errors.push({ emailName: searchParams.utm_term, expected: acc[param], actual: searchParams[param], param: param, resValue: true });
            }
        }
    ...
    return (
        <AppContext.Provider value={{ utmAcceptanceObj, parseUTMParameters, setUtmAcceptanceObj, getUTMAppendedLinks, utmAppendedLinks, results, loadings, setGotResult, gotResult, setLinks, setResults }}>
            {props.children}
        </AppContext.Provider>
    )
